...
export const editTask = async (req: Request, res: Response, next: NextFunction) => {
  ...
  const task = await AppDataSource.getRepository(Task).findOne({ where: { id: req.params.id }, relations: {
    testData: true,
    subtasks: true,
    developers: true,
    attachments: true
  ...
  if (!scripts[0].id) { // a new checkerScript was made
  ...
  if (scripts[1]) {
    if (!scripts[1].id && oldValidatorScipt) { // a new validatorScript was made
  ...
  for (const s of rbody.subtasks) {
    ...
    if (!scripts[scripts.length - 2].id && (await subtask.scorerScript).id) { // a new scorerScript was made
      toDelete.push(await subtask.scorerScript);
    }
    subtask.scorerScript = Promise.resolve(scripts[scripts.length - 2]);
  
    // resolve validatorScript
    if (!scripts[scripts.length - 1].id && (await subtask.validatorScript).id) { // a new validatorScript was made
