...
// We're also using this as validationEdit
export const validationCreate = async (req: Request, res: Response, next: NextFunction) => {
  ...
  for (let i = 0; i < rbody.attachments.length; i++) {
    ...
    if (validator.isEmpty(name)) {
      ...
      err.attachments[i].file.fileUrl = 'This field is required';
    } else {
      const file = files[name][0];
      rbody.attachments[i].file.fileUrl = file.path;
  ...
  for (let i = 0; i < rbody.subtasks.length; i++) {
    ...
    if (validator.isEmpty(scorerScriptName)) {
      ...
      err.subtasks[i].scorerScript.file.fileUrl = 'This field is required';
    } else {
      const file = files[scorerScriptName][0];
      rbody.subtasks[i].scorerScript.file.fileUrl = file.path;
    ...
    if (validator.isEmpty(validatorScriptName)){
      ...
      err.subtasks[i].validatorScript.file.fileUrl = 'This field is required';
    } else {
      const file = files[validatorScriptName][0];
      rbody.subtasks[i].validatorScript.file.fileUrl = file.path;
  ...
  for (let i = 0; i < rbody.data.length; i++) {
    ...
    if (validator.isEmpty(inputFileName)) {
      ...
      err.data[i].inputFile.fileUrl = 'This field is required';
    } else {
      const file = files[inputFileName][0];
      rbody.data[i].inputFile.fileUrl = file.path;
    ...
    if (validator.isEmpty(outputFileName)) {
      ...
      err.data[i].outputFile.fileUrl = 'This field is required';
    } else {
      const file = files[outputFileName][0];
      rbody.data[i].outputFile.fileUrl = file.path;
      rbody.data[i].outputFile.size = file.size;
    }
    
    if (!validator.isEmpty(judgeFileName)) {
      if (!(judgeFileName in files)) {
        err.data[i].judgeFile.fileUrl = 'This field is required';
      } else {
        const file = files[judgeFileName][0];
        rbody.data[i].judgeFile.fileUrl = file.path;
