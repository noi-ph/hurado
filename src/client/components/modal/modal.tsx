...
export function Modal({ show, onBackgroundClick, children }: ModalProps) {
  ...
  return ReactDOM.createPortal(
    <div className={classNames("absolute top-0 bottom-0 left-0 right-0 bg-black", !show && "hidden")} onClick={actualOnBackgroundClick}>
      <div className={classNames("modal", !show && "hidden")}>
        {children}
      </div>
    </div>,
    mount
