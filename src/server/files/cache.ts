...
export class FileCache {
  ...
  async put(storageType: string, container: string, filename: string, sourcePath: string): Promise<void> {
    if (this.maxSizeBytes <= 0) return;
  ...
  async putBuffer(storageType: string, container: string, filename: string, buffer: Buffer): Promise<void> {
    if (this.maxSizeBytes <= 0) return;
